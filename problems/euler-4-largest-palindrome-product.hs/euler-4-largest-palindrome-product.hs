...
eulerSolution digits = 
    let ceil = get10Exp digits
        floor = get10Exp (digits - 1)
        decNums = [ceil,ceil-1..floor]
        scope = [x*y | x <- decNums, y <- decNums]
    in maximum (filter isPalindrome scope) 
